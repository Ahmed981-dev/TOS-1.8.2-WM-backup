...
abstract class AudioRecorderBase(
    ...
    final override fun prepare() {
        bufferSizeInBytes =
            AudioRecord.getMinBufferSize(sampleRateInHz, channelConfig, audioEncoding)
        if (bufferSizeInBytes == AudioRecord.ERROR || bufferSizeInBytes == AudioRecord.ERROR_BAD_VALUE) {
            if (this.channelConfig == AudioFormat.CHANNEL_IN_STEREO) {
                ...
                throw RecorderException("Failed to get the minimum buffer size. ",
                    CodeError.ERROR_BUFFER_SIZE)
            }
        }

        try {
            audioRecord = AudioRecord(this.audioSource,
                this.sampleRateInHz,
                channelConfig,
                this.audioEncoding,
                bufferSizeInBytes)
